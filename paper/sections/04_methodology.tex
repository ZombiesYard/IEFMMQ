...
This chapter describes the methodological framework underlying the implemented system.
...
The procedure runtime is the central mechanism by which the proposed system tracks learner progress through the F/A-18C cold-start procedure and produces grounded, state-aware tutoring output.
...
Crucially, the resolver tracks unavailable or unknown sources, enabling downstream components to distinguish ``the value is zero'' from ``the value is unknown,'' which is essential for correct gating rule behaviour.
...
This summary is included in the help-cycle context, providing the language model with a concise representation of recent learner actions and state transitions without exposing raw high-frequency telemetry.
...
\subsection{Runtime Efficiency Caching}

% Evidence: core/llm_schema.py (lru_cache on build_help_response_schema), core/vision_facts.py (lru_cache on load_vision_facts_config)

Several performance-critical paths in the procedure runtime employ LRU caching to avoid repeated computation over long-running tutoring sessions. The HelpResponse JSON Schema, which validates every model-generated overlay and explanation against the current pack configuration, is cached keyed on the filesystem modification times of its source configuration files; schema regeneration is skipped when the configuration has not changed, eliminating redundant YAML parsing and JSON Schema construction on each help cycle. The vision facts configuration loader applies the same caching strategy, avoiding repeated parsing and normalisation of the 13-fact ontology on every visual observation merge. These caching points are applied selectively to paths that would otherwise repeat identical I/O and validation work, reflecting attention to runtime efficiency without introducing cache-invalidation complexity.
...
Knowledge grounding is a core architectural feature of the tutoring runtime: every help cycle assembles a grounding query from the current procedure state, retrieves relevant documentation snippets, and includes them as evidence context in the model prompt.
...
Documents are indexed at the sentence level: the F/A-18C NATOPS flight manual and supplemental cold-start documentation are preprocessed into a local JSON index.
...
The top-$k$ scoring snippets (default $k = 5$) are returned and included as evidence context in the help-cycle prompt.
...
DCS renders the three primary multifunction displays---left DDI, AMPCD, and right DDI---into a single composite-panel image.
The capture process triggers screenshots at configurable intervals, associates each capture with a session identifier and wall-clock timestamp, and stores the raw images alongside a sidecar manifest.
...
The reviewed JSONL is transformed into an OpenAI-compatible multimodal chat SFT format.
...
This chapter has described the four methodological pillars of the proposed system: telemetry-grounded procedure runtime, knowledge grounding with BM25 retrieval and source policy, the seven-stage visual fact data pipeline from screenshot capture to benchmark, and the LoRA fine-tuning protocol with matched hyperparameters across two backbone architectures.
