"""
Train google/gemma-4-31B with Unsloth on the SimTutor VLM SFT dataset.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fine-tune google/gemma-4-31B as a VLM with Unsloth."
    )
    parser.add_argument(
        "--train-jsonl",
        nargs="+",
    ...
    parser.add_argument(
        "--lora-target-modules",
        default="all-linear",
        help="Optional comma-separated LoRA target modules, for example 'all-linear'.",
    )
    parser.add_argument(
        "--finetune-vision-layers",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to apply LoRA to vision layers.",
    )
    parser.add_argument(
        "--load-in-4bit",
        action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="Unsloth GPU memory utilization hint passed to from_pretrained.",
    )
    parser.add_argument(
        "--chat-template",
        default="gemma-4",
        help="Optional Unsloth chat template name, for example 'gemma-4'.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Optional Trainer max_steps override for smoke tests; -1 means train by epochs.",
    )
...
def _apply_unsloth_chat_template(processor: Any, chat_template: str) -> Any:
    if not chat_template.strip():
        return processor
    from unsloth.chat_templates import get_chat_template

    template_name = chat_template.strip()
...
def train_gemma4_vlm_unsloth(
    *,
    ...
    if lora_target_modules.strip():
        stripped_target_modules = lora_target_modules.strip()
        if "," in stripped_target_modules:
            peft_kwargs["target_modules"] = [
                item.strip()
                for item in stripped_target_modules.split(",")
                if item.strip()
            ]
        else:
            peft_kwargs["target_modules"] = stripped_target_modules
...
def run_cli(argv: Sequence[str] | None = None) -> int:
    ...
    summary = train_gemma4_vlm_unsloth(
    ...
    print(
        "[TRAIN_GEMMA4_VLM_UNSLOTH] summary="
