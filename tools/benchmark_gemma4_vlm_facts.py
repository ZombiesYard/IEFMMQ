"""
Benchmark Gemma 4 VLM fact extraction before and after LoRA fine-tuning.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benchmark base vs LoRA Gemma 4 VLM visual fact extraction."
    )
...
def _run_predictions(
    *,
    ...
    for index, sample in enumerate(samples, start=1):
        ...
        print(
            "[BENCHMARK_GEMMA4_VLM_FACTS] item="
            + json.dumps(
                {
                    "model": model_label,
                    "index": index,
...
def benchmark_gemma4_vlm_facts(
    *,
...
def run_cli(argv: Sequence[str] | None = None) -> int:
    ...
    summary = benchmark_gemma4_vlm_facts(
    ...
    print(
        "[BENCHMARK_GEMMA4_VLM_FACTS] summary="
