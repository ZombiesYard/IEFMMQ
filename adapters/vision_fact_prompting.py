    ...
    *,
    ...
    if lang == "zh":
        rules = [
            ...
            "fcs_bit_result_visible 只有在右 DDI FCS-MC BIT 页面明确显示 FCS BIT 已完成的最终 GO 结果时才可为 seen。若能同时明确读到 FCSA=GO 与 FCSB=GO，可直接视为最终 GO 结果成立。若页面只显示 PBIT GO、FCSA/FCSB PBIT GO、NOT RDY、IN TEST 等中间状态，都不能算 fcs_bit_result_visible。",
        ...
        rules = [
            ...
            "fcs_bit_result_visible may be seen only when the right DDI FCS-MC BIT page clearly shows the final completed GO result. If the page clearly shows both FCSA=GO and FCSB=GO at the same time, treat that as sufficient final-GO evidence. Intermediate states such as PBIT GO, FCSA/FCSB PBIT GO, NOT RDY, or IN TEST do not satisfy fcs_bit_result_visible.",
